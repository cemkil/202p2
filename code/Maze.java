...
public class Maze {
  ...
  public boolean solveMaze(iSimpleContainer<Coordinate> sc, iDeque<Coordinate> visitedNodes, String mazeName)
    ...
    while (!sc.isEmpty()){
      Coordinate temp = sc.pop();
      if(isExit(temp.x , temp.y)){
        return true;
      }else if(!isVisited(temp)){
        markVisited(temp, visited);
        visitedNodes.addBehind(temp);
        Coordinate downCoordinate = new Coordinate(temp.x,temp.y-1);
        if(isInMaze(downCoordinate)){
          sc.push(downCoordinate);
        }
        Coordinate rightCoordinate = new Coordinate(temp.x+1,temp.y);
        if(isInMaze(rightCoordinate)){
          sc.push(rightCoordinate);
        }
        Coordinate upCoordinate = new Coordinate(temp.x,temp.y+1);
        if(isInMaze(upCoordinate)){
          sc.push(upCoordinate);
        }
        Coordinate leftCoordinate = new Coordinate(temp.x-1,temp.y);
        if(isInMaze(leftCoordinate)){
          sc.push(leftCoordinate);
        }
  ...
  private boolean isInMaze(Coordinate c) {
    try {
      if (currentMaze[c.x][c.y] == O || currentMaze[c.x][c.y] == E )
        return true;
    } catch (Exception e) {
      return false;
    }
